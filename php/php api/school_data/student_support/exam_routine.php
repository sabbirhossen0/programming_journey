...
function uploadPDF($file, $oldFile = null) {
    $targetDir = "uploads/exam_routine/";
...
if ($method === 'POST') {
    ...
    $stmt = $conn->prepare("INSERT INTO exam_routine (title, class, date, pdf) VALUES (?, ?, ?, ?)");
    $stmt->bind_param("siss", $title, $class, $date, $pdfName);

    if ($stmt->execute()) {
        http_response_code(201);
        echo json_encode(["status" => "success", "message" => "Exam routine created", "id" => $stmt->insert_id]);
    } else {
        http_response_code(500);
    ...
    $result = $conn->query("SELECT * FROM exam_routine ORDER BY class ASC");
    $routines = [];
    ...
    $stmt = $conn->prepare("UPDATE exam_routine SET title = ?, class = ?, date = ? WHERE id = ?");
    $stmt->bind_param("sisi", $title, $class, $date, $id);

    if ($stmt->execute()) {
        echo json_encode(["status" => "success", "message" => "Exam routine updated"]);
    } else {
        http_response_code(500);
        echo json_encode(["status" => "error", "message" => $conn->error]);
    }
    ...
    $res = $conn->query("SELECT pdf FROM exam_routine WHERE id = $id");
    if ($res && $res->num_rows > 0) {
        $row = $res->fetch_assoc();
        if ($row['pdf']) {
            $pdfPath = "uploads/exam_routine/" . $row['pdf'];
            if (file_exists($pdfPath)) unlink($pdfPath);
        }
    }

    $stmt = $conn->prepare("DELETE FROM exam_routine WHERE id = ?");
    $stmt->bind_param("i", $id);

    if ($stmt->execute()) {
        echo json_encode(["status" => "success", "message" => "Exam routine deleted"]);
    } else {
        http_response_code(500);
        echo json_encode(["status" => "error", "message" => $conn->error]);
    }
