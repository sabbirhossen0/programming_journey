...
if ($method === 'POST') {
    ...
    if (move_uploaded_file($_FILES['image']['tmp_name'], $targetFile)) {
        $imagePath = $imageName;  // store just filename
    } else {
    ...
    $stmt->bind_param("sssss", $imagePath, $title, $description, $activity_date, $year);

    if ($stmt->execute()) {
        echo json_encode(["status" => "success", "message" => "Yearly activity added successfully"]);
    } else {
    ...
    if ($id) {
        ...
        $conn->query("DELETE FROM yearly_activity WHERE id=$id");
